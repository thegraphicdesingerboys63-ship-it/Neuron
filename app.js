...
function linkify(text) {
  return escapeHtml(text)
    ...
    .replace(/@(\w{1,15})\b/g, (_, u) =>
      `<span class="mention${u === _currentUser?.username ? ' mention-me' : ''}">@${u}</span>`);
}

function _isMentioned(content) {
  if (!_currentUser) return false;
  return /@everyone\b/i.test(content) ||
    new RegExp(`@${_currentUser.username}\\b`, 'i').test(content);
}

// ─── Notifications & audio pings ─────────────────────────────────────────────
...
let _audioCtx = null;
function _playPing(type = 'dm') {
  try {
    if (!_audioCtx) _audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const ctx = _audioCtx;

    // DM / group ping: two quick rising tones
    // mention ping: three tones
    const tones = type === 'mention'
      ? [880, 1100, 1320]
      : [660, 880];
...
function _notify(title, body, tag) {
  ...
  if (!document.hidden && document.hasFocus()) return; // tab is active, skip
  try {
    const n = new Notification(title, {
      body,
      tag,           // same tag = replaces the previous one instead of stacking
...
function _getMentionCandidates() {
  if (_activeDmId) {
    // Use conversation members cached from last sidebar render
    const conv = _conversations.find(c => c.id === _activeDmId);
    if (conv) {
      // pull members from sidebar cache; fall back to usersCache keys
    }
  }
  const names = Object.keys(_usersCache);
...
function _initMentionAutocomplete() {
  ...
  input.addEventListener('input', () => {
    ...
    const candidates = _getMentionCandidates().filter(n =>
      n.toLowerCase().startsWith(query) && n !== _currentUser?.username
    ).slice(0, 8);

    if (candidates.length === 0) { dropdown.classList.add('hidden'); return; }

    dropdown.innerHTML = candidates.map(n =>
      `<div class="mention-option" data-name="${escapeHtml(n)}">@${escapeHtml(n)}</div>`
    ).join('');
    ...
    dropdown.querySelectorAll('.mention-option').forEach(opt => {
      opt.addEventListener('mousedown', e => {
        e.preventDefault();
        _insertMention(opt.dataset.name);
      });
    });
  });
...
function _insertMention(name) {
  const input    = document.getElementById('mention-dropdown');
...
function handleLogout() {
  ...
  clearInterval(_wsKeepaliveTimer); _wsKeepaliveTimer = null;
  clearInterval(_speakingInterval); _speakingInterval = null;
...
function replyTo(msgId) {
  const pool = _activeDmId ? _dmMessages : _lastMsgs;
...
async function renderMessages() {
  ...
  // Unread badge + notifications for new channel messages
  if (msgs.length > 0) {
    ...
    if (_lastSeenMsgId === null) {
      ...
      const newMsgs  = lastIdx === -1 ? msgs.slice(-1) : msgs.slice(lastIdx + 1);
      if (document.hidden && newMsgs.length > 0) {
        _unreadCount += newMsgs.length;
        _drawFavicon(_unreadCount);
      }
      // Ping + notify for mentions in new messages
      for (const m of newMsgs) {
        if (m.author === _currentUser?.username) continue;
        if (_isMentioned(m.content)) {
          _playPing('mention');
          _notify(
            `${m.author} mentioned you`,
            m.content.slice(0, 100),
            `mention-${m.id}`
          );
          break; // one ping per batch
        }
      }
      _lastSeenMsgId = newestId;
    }
  ...
  for (const msg of msgs) {
    ...
    html += `
      <div class="message ${isOwn ? 'own' : 'other'} ${isPinned ? 'pinned' : ''}${_isMentioned(msg.content) ? ' mentioned' : ''}" id="msg-${msg.id}">
...
function openReportModal(msgId) {
  const msg = (_activeDmId ? _dmMessages : _lastMsgs).find(m => m.id === msgId);
  if (!msg) return;
...
async function renderSidebarDms() {
  ...
  // Notify for conversations that gained unread messages since last render
  // (covers DMs that aren't currently open)
  for (const c of convs) {
    if (!c.unread || c.id === _activeDmId) continue;
    const prev = _conversations.find(p => p.id === c.id);
    if (!prev || c.unread > (prev.unread || 0)) {
      _playPing('dm');
      _notify(
        c.type === 'group' ? `New message in ${c.name}` : `New message from ${c.name}`,
        c.lastMsg || '',
        `conv-${c.id}`
      );
    }
...
async function pollDmMessages() {
  ...
  const cu        = _currentUser?.username;
  const convId    = _activeDmId;

  // Notify on new DM messages from others
  if (_dmMessages.length > 0 && msgs.length > _dmMessages.length) {
    const newMsgs = msgs.slice(_dmMessages.length);
    for (const m of newMsgs) {
      if (m.author === cu) continue;
      const isMention = _isMentioned(m.content);
      _playPing(isMention ? 'mention' : 'dm');
      const conv = _conversations.find(c => c.id === convId);
      const convName = conv
        ? (conv.type === 'dm' ? m.author : conv.name)
        : m.author;
      _notify(
        isMention ? `${m.author} mentioned you in ${convName}` : `${m.author} (${convName})`,
        m.content.slice(0, 100),
        `dm-${convId}`
      );
      break; // one ping per poll cycle
    }
  }
  ...
  for (const msg of msgs) {
    ...
    const isOwn    = msg.author === cu;
    const canDel   = isOwn || amIAdminOrAbove();
    const canReport = !isOwn;
    const mentioned = _isMentioned(msg.content);
    ...
    let contentHtml = '';
    if (msg.type === 'image' && msg.mediaUrl) {
      contentHtml = `<img src="${msg.mediaUrl}" class="msg-image" alt="image">`;
    } else if (msg.type === 'video' && msg.mediaUrl) {
      contentHtml = `<video src="${msg.mediaUrl}" class="msg-video" controls></video>`;
    } else {
      contentHtml = `<span>${linkify(msg.content)}</span>`;
    ...
    html += `
      <div class="message ${isOwn ? 'own' : 'other'}${mentioned ? ' mentioned' : ''}" id="dmsg-${msg.id}">
        <div class="message-bubble">
          <div class="message-meta">
          ...
          <div class="message-actions">
            <button class="action-btn" onclick="replyTo('${msg.id}')">Reply</button>
            ${canReport ? `<button class="action-btn" onclick="openReportModal('${msg.id}')">Report</button>` : ''}
            ${canDel    ? `<button class="action-btn danger" onclick="deleteDmMessage('${convId}','${msg.id}')">Delete</button>` : ''}
          </div>
        </div>
      </div>`;
  }
...
async function sendDmMessage() {
  ...
  if (!text) return;
  input.value = '';
  const replyTo = _replyToId || undefined;
  cancelReply();
  await api('POST', `/api/conversations/${_activeDmId}/messages`, { content: text, type: 'text', replyTo });
  await pollDmMessages();
  scrollBottom();
}
...
async function deleteDmMessage(convId, msgId) {
  ...
  await api('DELETE', `/api/conversations/${convId}/messages/${msgId}`);
  await pollDmMessages();
}
...
let _speakingInterval  = null;   // single interval id — prevents accumulation
let _wsKeepaliveTimer  = null;   // keepalive ping interval for Render
...
let _callIceBuffer    = {};    // username → RTCIceCandidate[] buffered before setRemoteDescription
...
function connectVoiceSocket() {
  ...
  _voiceWs.onopen = () => {
    ...
    // Keepalive ping every 25s — Render closes idle WS after ~60s
    clearInterval(_wsKeepaliveTimer);
    _wsKeepaliveTimer = setInterval(() => {
      if (_voiceWs?.readyState === 1) _voiceWs.send(JSON.stringify({ type: 'ping' }));
    }, 25000);
  };
...
function handleVoiceMessage(msg) {
  switch (msg.type) {
    ...
    case 'call-end':
      _hideIncomingCall(); // covers case where we haven't answered yet
...
const _voiceIceBuffer = {}; // username → RTCIceCandidate[] for public voice rooms

async function _handleOffer(from, sdp) {
  const pc = _newPeerConnection(from);
  await pc.setRemoteDescription(new RTCSessionDescription(sdp));
  for (const c of (_voiceIceBuffer[from] || [])) {
    try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch {}
  }
  delete _voiceIceBuffer[from];
...
async function _handleIce(from, candidate) {
  ...
  if (!pc || !pc.remoteDescription) {
    if (!_voiceIceBuffer[from]) _voiceIceBuffer[from] = [];
    _voiceIceBuffer[from].push(candidate);
    return;
  }
...
function _cleanupVoice() {
  ...
  // Only stop the stream if not in a private call
  if (!_activeCallConvId && _localStream) {
    _localStream.getTracks().forEach(t => t.stop());
    _localStream = null;
  }
  // Stop speaking detection only if no call active either
  if (!_activeCallConvId && _speakingInterval) {
...
async function startCall() {
  ...
  // convName shown to callee: for DM show caller name, for group show group name
...
function _cleanupCall() {
  ...
  _callParticipants = [];
  _callIceBuffer    = {};
  // Only stop stream/detection if not also in a voice room
  if (!_voiceRoom) {
...
function _startRingtone() {
  _stopRingtone();
  // Play immediately then repeat every 3s
  _playRingTone();
  _ringtoneInterval = setInterval(_playRingTone, 3000);
}
...
function _playRingTone() {
  try {
    if (!_audioCtx) _audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const ctx = _audioCtx;
    // Two-burst pattern: high-low-high-low, like a phone ring
    const pattern = [
      { freq: 480, t: 0,    dur: 0.2 },
      { freq: 440, t: 0.22, dur: 0.2 },
      { freq: 480, t: 0.5,  dur: 0.2 },
      { freq: 440, t: 0.72, dur: 0.2 },
    ];
    const now = ctx.currentTime;
    for (const { freq, t, dur } of pattern) {
      const osc  = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.connect(gain);
      gain.connect(ctx.destination);
      ...
      gain.gain.linearRampToValueAtTime(0.3, now + t + 0.01);
...
function _updateCallUI() {
  ...
  if (!bar) return;
  if (_activeCallConvId) {
    // Label: try to get conversation name from cache
    const conv = _conversations.find(c => c.id === _activeCallConvId);
    let label = 'Call';
    if (conv) {
      const me = _currentUser?.username;
      label = conv.type === 'dm'
        ? conv.members?.find?.(m => m !== me) || conv.name
        : conv.name;
    } else if (_callParticipants.length > 0) {
      label = _callParticipants.join(', ');
    }
    document.getElementById('csb-name').textContent = label;
...
function toggleCallMute() {
  ...
  const btn = document.getElementById('csb-mute-btn');
  if (btn) {
    btn.textContent = _isMuted ? '🔇' : '🎤';
    btn.classList.toggle('muted', _isMuted);
  }
  // Also sync the voice room mute button
  const vsbBtn = document.getElementById('vsb-mute-btn');
  if (vsbBtn) {
    vsbBtn.textContent = _isMuted ? '🔇' : '🎤';
    vsbBtn.classList.toggle('muted', _isMuted);
  }
}
...
async function _callHandleOffer(from, sdp) {
  ...
  await pc.setRemoteDescription(new RTCSessionDescription(sdp));
  // Flush buffered ICE candidates that arrived before this offer
  for (const c of (_callIceBuffer[from] || [])) {
    try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch {}
  }
  delete _callIceBuffer[from];
  const answer = await pc.createAnswer();
  await pc.setLocalDescription(answer);
  _voiceWs.send(JSON.stringify({
...
async function _callHandleAnswer(from, sdp) {
  ...
  if (!pc) return;
  await pc.setRemoteDescription(new RTCSessionDescription(sdp));
  // Flush buffered ICE candidates that arrived before this answer
  for (const c of (_callIceBuffer[from] || [])) {
    try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch {}
  }
  delete _callIceBuffer[from];
...
async function _callHandleIce(from, candidate) {
  ...
  if (!pc || !pc.remoteDescription) {
    ...
    if (!_callIceBuffer[from]) _callIceBuffer[from] = [];
    _callIceBuffer[from].push(candidate);
    return;
  }
  try { await pc.addIceCandidate(new RTCIceCandidate(candidate)); } catch {}
}
...
function _startSpeakingDetection(stream) {
  // Clear any existing interval — prevents accumulation on rejoin
  if (_speakingInterval) { clearInterval(_speakingInterval); _speakingInterval = null; }
  try {
    ...
    _speakingInterval = setInterval(() => {
      if (!_voiceRoom && !_activeCallConvId) return; // not in any voice session
      analyser.getByteFrequencyData(buf);
      const vol      = buf.reduce((a, b) => a + b, 0) / buf.length;
      const speaking = vol > 10 && !_isMuted;
      if (speaking !== wasSpeaking) {
        wasSpeaking = speaking;
        if (_voiceRoom && _voiceWs?.readyState === 1) {
...
async function joinVoice(roomId) {
  ...
  // If WS is still connecting, wait up to 4s for it to open
  if (!_voiceWs || _voiceWs.readyState > 1) connectVoiceSocket();
  if (_voiceWs.readyState === 0) {
    await new Promise((resolve, reject) => {
      const t = setTimeout(() => reject(), 4000);
      _voiceWs.addEventListener('open', () => { clearTimeout(t); resolve(); }, { once: true });
      _voiceWs.addEventListener('error', () => { clearTimeout(t); reject(); }, { once: true });
    }).catch(() => {
      alert('Could not connect to voice server. Try again in a moment.'); return;
    });
    if (_voiceWs.readyState !== 1) return;
  }

  // Request microphone (reuse existing stream if we have one)
  if (!_localStream) {
    try {
      _localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
    } catch {
...
function toggleMute() {
  ...
  const icon = _isMuted ? '🔇' : '🎤';
  for (const id of ['vsb-mute-btn', 'csb-mute-btn']) {
    const btn = document.getElementById(id);
    if (btn) { btn.textContent = icon; btn.classList.toggle('muted', _isMuted); }
  }
}
