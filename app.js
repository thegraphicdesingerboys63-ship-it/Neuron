...
function makeMsgEl(m, grouped) {
  ...
  if (grouped) {
    ...
    d.innerHTML = `
      <div class="msg-avatar">${escapeHtml(displayName[0].toUpperCase())}</div>
      <div class="msg-content-wrap">
        <div class="msg-header">
          <span class="msg-author" style="color:${roleColor}" onclick="showUserDetails('${escapeHtml(m.author)}')">${escapeHtml(displayName)}</span>
          ${m.display_name ? `<span class="msg-username-tag">@${escapeHtml(m.author)}</span>` : ''}
...
async function loadFriends() {
  ...
  friends        = data.friends  || [];   // [{ id, username, acceptedAt }]
  friendRequests = data.incoming || [];   // [{ id, username, createdAt }]
  renderFriends();
}
  ...
  if (friendRequests.length) {
    ...
    friendRequests.forEach(f => {
      html += `<div class="friend-row">
        <div class="friend-avatar">${escapeHtml(f.username[0].toUpperCase())}</div>
        <span class="friend-name">${escapeHtml(f.username)}</span>
        <div class="friend-actions">
          <button onclick="respondFriend('${escapeHtml(f.username)}','accept')">✓</button>
          <button onclick="respondFriend('${escapeHtml(f.username)}','reject')">✕</button>
        </div>
      </div>`;
    });
  }
  if (friends.length) {
    ...
    friends.forEach(f => {
      html += `<div class="friend-row">
        ...
        <span class="friend-name">${escapeHtml(f.username)}</span>
        <div class="friend-actions">
          <button onclick="openDmWith('${escapeHtml(f.username)}')">DM</button>
          <button onclick="removeFriend('${escapeHtml(f.username)}')">✕</button>
        </div>
      </div>`;
    });
  }
