...
app.get('/api/conversations/:id/messages', auth, async (req, res) => {
  try {
    ...
    res.json(rows.map(m => ({ id: m.id, author: m.author, content: m.content, type: m.type, mediaUrl: m.media_url, timestamp: m.timestamp, flagged: !!m.flagged })));
  } catch (e) { res.status(500).json({ error: 'Server error.' }); }
});
...
app.post('/api/conversations/:id/messages', auth, async (req, res) => {
  try {
    ...
    const id = uid();
    await db.execute({
      sql: 'INSERT INTO conv_messages (id, conv_id, author, content, type, media_url, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)',
      args: [id, req.params.id, me, content, type || 'text', mediaUrl || null, new Date().toISOString()],
    });
    await db.execute({ sql: 'UPDATE conv_members SET last_read_at = ? WHERE conv_id = ? AND username = ?', args: [new Date().toISOString(), req.params.id, me] });
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'voice-speaking') {
      ...
      for (const [mws] of room) {
        const mwsObj = [...room.values()].find((_, i) => [...room.keys()][i] === mws);
