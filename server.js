...
// Platform roles: user < mod < manager < admin < owner < supreme
const RANK = { user: 0, mod: 1, manager: 2, admin: 3, owner: 4, supreme: 5 };
...
const requireManagerOrAbove = requireRole('manager', 'admin', 'owner', 'supreme');
const requireModOrAbove     = requireRole('mod', 'manager', 'admin', 'owner', 'supreme');
...
app.patch('/api/me/profile', auth, async (req, res) => {
  try {
    const { display_name, username: newUsername } = req.body ?? {};
    const user = await getUser(req.user.username);
    if (!user) return res.status(404).json({ error: 'User not found.' });

    if (display_name !== undefined) {
      const dn = display_name.trim().slice(0, 32);
      await db.execute({ sql: 'UPDATE users SET display_name = ? WHERE username = ?', args: [dn || null, user.username] });
    }

    if (newUsername !== undefined) {
      const nu = newUsername.trim();
      if (!/^[a-zA-Z0-9_]{3,15}$/.test(nu)) return res.status(400).json({ error: 'Username must be 3–15 letters, numbers, underscores.' });
      if (nu !== user.username) {
        if (RESERVED.includes(nu)) return res.status(400).json({ error: 'That username is reserved.' });
        const taken = await getUser(nu);
        if (taken) return res.status(400).json({ error: 'Username already taken.' });
        await db.execute({ sql: 'UPDATE users SET username = ? WHERE username = ?', args: [nu, user.username] });
        // issue new token with updated username
        const newToken = jwt.sign({ username: nu, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
        return res.json({ ok: true, new_token: newToken, username: nu });
      }
    }
    res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Server error.' }); }
...
app.get('/api/channels/:channelId/messages', auth, async (req, res) => {
  try {
    ...
    let sql, args;
    if (after) {
      sql  = `SELECT m.*, u.role AS author_role, u.display_name AS author_display_name FROM server_messages m LEFT JOIN users u ON u.username = m.author WHERE m.channel_id = ? AND m.deleted = 0 AND m.timestamp > ? ORDER BY m.timestamp ASC LIMIT 100`;
      args = [req.params.channelId, after];
    } else if (before) {
      sql  = `SELECT m.*, u.role AS author_role, u.display_name AS author_display_name FROM server_messages m LEFT JOIN users u ON u.username = m.author WHERE m.channel_id = ? AND m.deleted = 0 AND m.timestamp < ? ORDER BY m.timestamp DESC LIMIT 100`;
      args = [req.params.channelId, before];
    } else {
      sql  = `SELECT m.*, u.role AS author_role, u.display_name AS author_display_name FROM server_messages m LEFT JOIN users u ON u.username = m.author WHERE m.channel_id = ? AND m.deleted = 0 ORDER BY m.timestamp ASC LIMIT 100`;
      args = [req.params.channelId];
    }

    const rows = (await db.execute({ sql, args })).rows;
    const vr   = req.user.role;
    const msgs = (before ? rows.reverse() : rows).map(m => ({
      id: m.id, author: m.author,
      display_name: m.author_display_name || null,
      content: m.content, type: m.type,
      media_url: m.media_url, link_url: m.link_url, reply_to: m.reply_to,
      pinned: !!m.pinned, edited_at: m.edited_at, timestamp: m.timestamp,
      author_role: safeRole(m.author_role, vr),
    }));
...
app.post('/api/channels/:channelId/messages', auth, async (req, res) => {
  try {
    ...
    const { content, type, media_url, link_url, reply_to } = req.body ?? {};
    if (!content && type !== 'image') return res.status(400).json({ error: 'Content required.' });
    const msgContent = content || media_url || '';
    if (msgContent.length > maxLen) return res.status(400).json({ error: `Message too long (max ${maxLen}).` });

    const id = uid();
    await db.execute({
      sql: 'INSERT INTO server_messages (id,channel_id,server_id,author,content,type,media_url,link_url,reply_to,timestamp) VALUES (?,?,?,?,?,?,?,?,?,?)',
      args: [id, req.params.channelId, ch.server_id, req.user.username, msgContent, type || 'text', media_url || null, link_url || null, reply_to || null, new Date().toISOString()],
    });
...
app.get('/api/admin/users', auth, requireManagerOrAbove, async (req, res) => {
  try {
    const vr = req.user.role;
...
app.patch('/api/admin/users/:username', auth, requireManagerOrAbove, async (req, res) => {
  try {
    ...
    const needsAdmin   = () => { if (roleRank(cu.role) < 3) return res.status(403).json({ error: 'Admin required.' }); };
    const needsOwner   = () => { if (roleRank(cu.role) < 4) return res.status(403).json({ error: 'Owner required.' }); };
    ...
    switch (action) {
      ...
      case 'set_role': {
        const VALID = ['user','mod','manager','admin','owner'];
        if (!VALID.includes(role)) return res.status(400).json({ error: 'Invalid role.' });
        if (roleRank(role) >= 3) { const r = needsOwner(); if (r) return; }
        if (roleRank(role) >= 2) { const r = needsAdmin(); if (r) return; }
        if (role === 'owner' && cu.role !== 'supreme') return res.status(403).json({ error: 'Supreme only.' });
        await db.execute({ sql: 'UPDATE users SET role = ? WHERE username = ?', args: [role, target.username] }); break;
      }
      case 'add_note': {
        const r = needsAdmin(); if (r) return;
        await db.execute({ sql: 'UPDATE users SET notes = ? WHERE username = ?', args: [note ?? '', target.username] }); break;
      }
      case 'clear_messages': {
        const r = needsOwner(); if (r) return;
...
app.get('/api/admin/ai-flags', auth, requireManagerOrAbove, async (req, res) => {
  try {
...
app.get('/api/reports', auth, requireModOrAbove, async (req, res) => {
  try {
    ...
    res.json({ reports: (await db.execute({ sql, args: [] })).rows });
  } catch (e) { res.status(500).json({ error: 'Server error.' }); }
});
...
app.get('/api/conversations/:id/messages', auth, async (req, res) => {
  try {
    ...
    res.json({ messages: rows.map(m => ({ id: m.id, author: m.author, content: m.content, type: m.type, media_url: m.media_url, reply_to: m.reply_to, timestamp: m.timestamp })) });
  } catch (e) { res.status(500).json({ error: 'Server error.' }); }
});
